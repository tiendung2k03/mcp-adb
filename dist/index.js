...
// --- Core ADB Tools ---
server.tool("adb_devices", "Lists all connected Android devices and emulators.", {}, () => executeCommandAsTool("adb devices -l"));
server.tool("adb_shell", "Executes a shell command on the connected Android device.", {
...
server.tool("inspect_ui", "Captures the complete UI hierarchy of the current screen as an XML document.", {
    device: z.string().optional().describe("Optional device ID"),
}, async ({ device }) => {
    const deviceArg = device ? `-s ${device} ` : "";
...
server.tool("screenshot", "Takes a screenshot of the current screen and returns it as a base64 string.", {
    device: z.string().optional().describe("Optional device ID"),
}, async ({ device }) => {
    const deviceArg = device ? `-s ${device} ` : "";
...
try {
    ...
    if (fs.existsSync(commandsDir)) {
        ...
        for (const file of files) {
            ...
            server.tool(toolName, shape, (args) => {
